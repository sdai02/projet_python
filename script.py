...
class Answer:
    ...
    def send_receive(self, message, split_last=False):
        ...
        return check.split()[-1] if split_last else check  # Return the full response or the last word
    ...
    def solve_math(self, response):
        ...
        match = re.search(r"(\d+)\s*([\+\-\*])\s*(\d+)", response)  # Search for a math operation
        if match:
            num1, operator, num2 = (
                int(match.group(1)),
                match.group(2),
            ...
            self.save_answers(str(result))  # Save the result in the file
            with open("question_3.txt", "w") as file:
    ...
    def decode_base(self, response):
        ...
        for decode in encodings:
            try:
                decoded_message = decode(response.encode()).decode()  # Decode the message
                self.save_answers(str(decoded_message))  # Save the decoded message
                return self.send_receive(
                    str(decoded_message), split_last=True
                )  # Return the decoded message
            except Exception:
                continue
        return self.reset_answers_file()  # If decoding fails, reset the answers file
    ...
    def braille(self, braille_message):
        ...
        self.save_answers(str(new_braille_message))  # Save the decoded braille message
    ...
    def colors(self, response):
        ...
        if match:
            ...
            self.save_answers(str(color_name))  # Save the color name
    ...
    def rewind(self, response):
        ...
        with open("answers.txt", "r") as file:
            ...
            if line_number <= len(lines):
                ...
                self.save_answers(str(line))  # Save the returned line
                return self.send_receive(line)

    def find(self, response):
        """Search for a specific word in a list"""
        match = re.findall(r"[1-7](?=ème)", response)  # Search for numbers 1-7 followed by "ème"
        if match:
            ...
            self.save_answers(last_letter)  # Save the last letter
    ...
    def all_answers(self):
        ...
        formatted_response = "_".join(response.strip() for response in responses)  # Format responses
        return self.send_receive(formatted_response, split_last=True)  # Send the formatted response
...
def reader():
    ...
    with open("counter.txt", "r") as file:
        ...
        return int(content) if content.isdigit() else 10000  # Return 10000 if counter is invalid
...
def main():
    ...
    responses = ["Stephane/dai/3SI5", datetime.datetime.now().strftime("%d/%m"), "", ""]  # Initial responses
    ...
    for message in responses:
        ...
        if message == "Stephane/dai/3SI5":
            answer.save_answers("Stephane/dai/3SI5")
            answer.save_answers(datetime.datetime.now().strftime("%d/%m"))
